...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, df, transform = None):
        self.df = df
        self.images_folder = "/home/input/kinetosis-utkf/dataset/images_utk"
...
print("starting...")
df = pd.read_csv("/home/input/kinetosis-utkf/dataset/csv/dataset_UTK1.csv")
...
def save_checkpoint(model, epoch, optimizer, best_acc):
    ...
    torch.save(state, '/home/working/model_best_checkpoint_modified_batch32.pth')

# define the parameter for training and validation
model.fc = ageGenderClassifier().to(device)
...
for epoch in range(n_epochs):
    ...
    with open('/home/working/age_modified_batch32.log', 'a') as f:
...
im = cv2.imread("/home/input/kinet-test/test/file_5.jpg")
im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
...
im = cv2.imread("/home/input/kinet-test/test/file_131.jpg")
im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
im = cv2.resize(im,(224,224)) / 255
im = torch.tensor(im).permute(2,0,1).to(device)
im = train_dataset.normalize(im)
...
im = cv2.imread("/home/input/kinet-test/test/file_23.jpg")
im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
im = cv2.resize(im,(224,224)) / 255
im = torch.tensor(im).permute(2,0,1).to(device)
im = train_dataset.normalize(im)
