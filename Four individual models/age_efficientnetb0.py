...
# get Efficientnetb0 pretrained model
import torchvision.models as models
...
def save_checkpoint(model, epoch, optimizer, best_acc):
    ...
    torch.save(state, '/kaggle/working/model_efficientnetb0_best_checkpoint_modified_batch32.pth')
...
for epoch in range(n_epochs):
    ...
    with open('/kaggle/working/age_efficientnetb0_modified_batch32.log', 'a') as f:
...
im = cv2.imread("/kaggle/input/kinet-test/test/file_131.jpg")
im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
im = cv2.resize(im,(224,224)) / 255
im = torch.tensor(im).permute(2,0,1).to(device)
im = train_dataset.normalize(im)
