...
def save_checkpoint(model, epoch, optimizer, best_acc):
    ...
    torch.save(state, 'model_best_checkpoint_gen.pth')

# define the parameter for training and validation
model.classifier = genderClassifier().to(device)
...
for epoch in range(n_epochs):
    ...
    with open('/home/chandhinii/kinetosis/gender_batch32.log', 'a') as f:
...
im = cv2.imread("/app/kinetosis/test/file_5.jpg")
im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
...
im = cv2.imread("/app/kinetosis/test/file_1.jpg")
im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
im = cv2.resize(im,(224,224)) / 255
im = torch.tensor(im).permute(2,0,1).to(device)
im = train_dataset.normalize(im)
...
im = cv2.imread("/app/kinetosis/test/file_23.jpg")
im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
im = cv2.resize(im,(224,224)) / 255
im = torch.tensor(im).permute(2,0,1).to(device)
im = train_dataset.normalize(im)
